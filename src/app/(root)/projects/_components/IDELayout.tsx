...
export default function IDELayout({ code, setCode }: any) {
  ...
  const [activeTab, setActiveTab] = useState<"editor" | "canvas">("editor"); // 👈 manual tab control
  ...
  return (
    <div className="h-full w-full bg-slate-900 text-white flex flex-col">
      ...
      <div className="flex gap-2 p-2 bg-slate-800 border-b border-gray-700">
        <button
          className={`px-3 py-1 rounded ${activeTab === "editor" ? "bg-blue-600" : "bg-gray-700"}`}
          onClick={() => setActiveTab("editor")}
        >
          Editor
        </button>
        <button
          className={`px-3 py-1 rounded ${activeTab === "canvas" ? "bg-blue-600" : "bg-gray-700"}`}
          ...
          <PanelGroup direction="horizontal">
            ...
            <Panel defaultSize={30} minSize={20}>
              <Card className="h-full bg-slate-950 p-4">
                <h2 className="text-lg font-bold mb-2">🤖 Sentiment Bot</h2>
                <div className="h-full flex items-center justify-center text-gray-500">
                  (Future Sentiment Analysis UI)
                </div>
              </Card>
            </Panel>
          </PanelGroup>
